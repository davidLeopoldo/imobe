...
export async function gerarContratoVinculadoAction(
  imovelId: number,
  input: ContratoFormValues
) {
  ...
  try {
    const contrato = await gerarContratoComPdf(
      mapContratoFormValuesToInput(parsed.data, imovelId)
    );
